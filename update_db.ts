...
async function update() {
  ...
  if (admins) {
    for (const admin of admins) {
      if (admin.email.includes('malabaz.com')) {
        const newEmail = admin.email.replace('malabaz.com', 'malabez.com');
        await supabase.from('admins').update({ email: newEmail }).eq('id', admin.id);
        console.log(`Updated admin ${admin.email} to ${newEmail}`);
      }
    }
  }
}
