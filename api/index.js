...
app.delete("/api/admin/admins/:id", async (req, res) => {
  ...
  // First delete support messages sent by this admin
  await supabase.from('support_messages').delete().eq('sender_id', id).eq('sender_type', 'admin');
...
app.delete("/api/admin/products/:id", async (req, res) => {
  const { id } = req.params;
  // First delete order items referencing this product to avoid foreign key constraint errors
...
app.delete("/api/admin/categories/:id", async (req, res) => {
  ...
  if (category) {
    ...
    await supabase.from('products').update({ category: null }).eq('category', category.name);
  }
